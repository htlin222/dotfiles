...
def find_existing_note(front):
    payload = {
        ...
        "version": 6,
        "params": {
            "query": f'{ANKI_FRONT_FIELD}:"{front}"'
        },
    }
...
def main():
    ...
    if front_matter is None:
        print("Could not find front matter in the markdown file.")
        return

    # Check if 'anki' is in the tags
    # tags = front_matter.get("tags", [])
    # if "anki" not in [tag.lower() for tag in tags]:
    #     print("Anki tag not found in front matter.")
    #     return

    # Step 3: Extract 'front' and 'back'
    front = front_matter.get("description", None)
    if front is None:
        front = front_matter.get("title", None)

    if front is None and "#" in content:
        front = content.split("#")[1].split("\n")[0].strip()

    back = content.split("#")[1] if "#" in content else content
