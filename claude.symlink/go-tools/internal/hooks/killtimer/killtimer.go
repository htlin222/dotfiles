...
func Start(claudePID int, sessionID string) error {
	...
	// 1. Sleep until 1 min before deadline, play warning sound
	...
	script := fmt.Sprintf(
		"sleep %d && afplay /System/Library/Sounds/Ping.aiff 2>/dev/null; sleep 60 && kill -TERM %d 2>/dev/null; sleep 5 && kill -KILL %d 2>/dev/null; afplay /System/Library/Sounds/Hero.aiff &",
