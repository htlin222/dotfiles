...
description: Analyze Claude Code context efficiency for current project
---

# Context Efficiency Report

Analyze the Claude Code JSONL session data for the **current project** and generate a context efficiency report.

## Project Path Detection

The current working directory is: `$ARGUMENTS` (if provided) or use `$CWD`
...
if [ "$model_total" -gt 0 ]; then
  ...
  [ "$opus_pct" -gt 80 ] && warnings="${warnings}⚠️  Opus ${opus_pct}% - use --model haiku for exploration\n"
else
  opus_pct=0
  model_score=15
  score=$((score + model_score))
...
if [ "$session_count" -gt 0 ]; then
  ...
  [ "$delegation_ratio_x100" -lt 100 ] && warnings="${warnings}⚠️  Low delegation - use Task agents more\n"
else
  delegation_score=0
fi
...
# 4. TodoWrite score (20 pts): using task tracking
if [ "$msg_count" -gt 0 ]; then
  ...
  [ "$todo_count" -eq 0 ] && warnings="${warnings}⚠️  No TodoWrite - use for task tracking\n"
else
  todo_score=0
fi
...
printf "    Agent delegation:    %2d/25  (ratio %s:1)\n" "$delegation_score" "$del_ratio"
...
Present the results clearly and provide actionable recommendations based on:

1. Model usage distribution (aim for <50% Opus)
2. Tool efficiency (Grep > bash grep, Glob > bash find)
3. Agent delegation ratio (aim for >2:1)
