...
- 考慮使用 [shellcheck](https://www.shellcheck.net/) 工具檢查腳本常見錯誤
...
**背景（問題發現）**：在文件、README 或技術部落格中展示操作流程時，GIF 動畫比靜態截圖更直觀。但 [ffmpeg](https://ffmpeg.org/) 的參數複雜，每次都要查文件才能記得正確的轉換命令。
...
- 替代工具：[gifsicle](https://www.lcdf.org/gifsicle/) 可進一步壓縮 GIF、[gifski](https://gif.ski/) 提供更高品質的轉換
...
**方法**：使用 [Click](https://click.palletsprojects.com/) 框架快速建立 Python CLI 工具：
...
- 替代框架：[argparse](https://docs.python.org/3/library/argparse.html)（標準庫）、[typer](https://typer.tiangolo.com/)（基於型別提示）、[fire](https://github.com/google/python-fire)（自動生成 CLI）
- 打包發布：使用 [setuptools](https://setuptools.pypa.io/) 或 [poetry](https://python-poetry.org/) 將工具打包成可安裝的套件
