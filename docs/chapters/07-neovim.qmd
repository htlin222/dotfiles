...
## 打造個人 Wiki：Markdown 編輯的魔法

如果你像我一樣用 Markdown 來管理筆記、知識庫或個人 Wiki，以下是一些讓編輯體驗更順暢的設定技巧。

### Wikilink 與自動模板

你有沒有想過：在特定資料夾建立新的 `.md` 檔案時，自動幫你填好 frontmatter 和反向連結？

我的做法是在 Wiki 目錄放一個 `.iswiki` 標記檔，當 Neovim 偵測到這個標記，建立新檔案時就會自動套用模板：
...
這裡有個小巧思：`vim.g.previous` 會在你離開一個 Markdown 檔案時自動記錄檔名，所以新建的筆記會自動包含「來自哪裡」的反向連結，形成知識網絡。

### 快速操作 Wikilink

編輯 Wiki 時，最常做的事就是建立連結。這幾個快捷鍵讓我省下大量時間：

| 快捷鍵 | 功能 | 說明 |
|--------|------|------|
| `<leader>yw` | 複製為 wikilink | 把當前檔名變成 `[[filename]]` 格式複製 |
| `<leader>pw` | 貼上為 wikilink | 把剪貼簿內容包上 `[[]]` 貼上 |
| `<leader>of` | 插入「of this file」 | 快速建立子筆記的命名後綴 |
...
[mkdnflow.nvim](https://github.com/jakewvincent/mkdnflow.nvim) 是讓 Neovim 變成 Wiki 編輯器的關鍵：
