...
EAGER_PLUGINS=(
  ...
  fast-syntax-highlighting  # 語法高亮（需立即可見）
)

# 第一次顯示 prompt 時載入（約 100ms 後）
PROMPT_PLUGINS=(
  zsh-autosuggestions       # 命令自動建議
)
...
需要搭配延遲載入框架（如 `zsh-defer` 或 `zsh-lazyload`）才能實現：
...
Powerlevel10k 是目前最快的 Zsh 主題，採用以下技術：
